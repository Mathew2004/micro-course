...
if(isset($_POST['submit'])){
  ...
  if(!empty($name)){
   ...
   $phn = $_POST['phone'];
   $phn = filter_var($phn, FILTER_SANITIZE_STRING);

  if(!empty($phn)){
   $update_phn = $conn->prepare("UPDATE `users` SET phone = ? WHERE id = ?");
   $update_phn->execute([$phn, $user_id]);
  ...
  if(!empty($batch)){
   $update_batch = $conn->prepare("UPDATE `users` SET batch = ? WHERE id = ?");
   $update_batch->execute([$batch, $user_id]);
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <div class="flex">
         <div class="col">
            ...
            <p>Phone</p>
            <input type="tel" name="phone" placeholder="<?= $fetch_profile['phone']; ?>" maxlength="100" class="box">
            <p>Batch</p>
            <select name="batch" class="box" required>
                  <option value="<?= $fetch_profile['batch']; ?>" selected ><?= $fetch_profile['batch']; ?></option>
