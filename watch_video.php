...
?>
   ...
   <div class="video-flex">
      ...
      ?>
         <div class="video-details">
            ...
            <iframe width="780" height="500"
               src="https://www.youtube.com/embed/<?= $fetch_content['video']; ?>?&autoplay=1" frameborder="0"
            ...
            <div class="info">
               ...
               <p><i class="fas fa-heart"></i><span id="totalLike"></span></p>
            </div>
            ...
            <div class="flex">
               ...
               <?php 
               if($fetch_content['lecsheet'] == 'null'){
                  echo "<p class='empty'>Lecture Sheet will comming soon!</p>";
               }
               else{
               ?>
               <a href="<?= $fetch_content['lecsheet']; ?>" target="_blank" class="inline-btn">Lecture Sheet</a>
               <?php } 
         // if($verify_likes->rowCount() > 0){
            ?>
               ...
               <!---- <button type="submit" name="like_content" id='like'><i class="fas fa-heart"></i><span>liked</span></button>
            <?php
            //}else{
            ?>
            <button type="submit" name="like_content" id='like'><i class="far fa-heart"></i><span>like</span></button>
            <?php
               //}
            ?>  -->
            </div>
      ...
      <div class="live-viewers">
         ...
         <div class="video-list-container">
            ...
            <?php
               $select_content = $conn->prepare("SELECT * FROM `content` WHERE playlist_id = ?  ORDER BY date ASC");
               $select_content->execute([$playlist_id]);
               if($select_content->rowCount() > 0){
                  while($fetch_content = $select_content->fetch(PDO::FETCH_ASSOC)){ 
                     $status = "";
                     if($fetch_content['id'] == $get_id){
                        $status = "active";
                     } 
            ?>
            <a href="/watch_video.php?get_id=<?=$fetch_content['id']; ?>">
            <div class="list <?php echo $status; ?>" >
               <img src="<?= $fetch_content['thumb']; ?>" class="list-video">
               <h3 class="list-title">
                  <?= $fetch_content['title']; ?>
               </h3>
            </div></a>
   ...
   <script>
      $(document).ready(function () {
         ...
         function load_like(content_id) {
            $.ajax({
               url: "ajax/load_like.php",
               type: "POST",
               data: { id: content_id },
               success: function (data) {
                  $('#updated_like').html(data);
                  //  alert(data);

               }
            })
         }
         function TotalLike(content_id) {
            $.ajax({
               url: "ajax/totalLikes.php",
               type: "POST",
               data: { id: content_id },
               success: function (data) {
                  $('#totalLike').html(data);
